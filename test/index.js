...
describe('detect type', function () {
  it('It should return undefined', function () {
    assert.deepStrictEqual(api._type(), 'undefined')
    assert.deepStrictEqual(api._type({}.x), 'undefined')
    assert.deepStrictEqual(api._type(this.y), 'undefined')
  })

  it('It should return array', function () {
    assert.deepStrictEqual(api._type([]), 'array')
  })

  it('It should return object', function () {
    assert.deepStrictEqual(api._type({}), 'object')
    assert.deepStrictEqual(api._type(Object.create(null)), 'object')
    assert.deepStrictEqual(api._type(Object()), 'object')
  })
})

describe('param url', function () {
  it('It should return a tring', function () {
    assert.deepStrictEqual(typeof api._param({}), 'string')
  })
  it('It could ignore string and number', function () {
    assert.deepStrictEqual(api._param({name: '6'}), 'name=6')
    assert.deepStrictEqual(api._param({name: 6}), 'name=6')
  })
  it('It should encodeURIComponent keys and values', function () {
    assert.deepStrictEqual(api._param({name: '1 2'}), 'name=1%202')
    assert.deepStrictEqual(api._param({'a,b': 6}), 'a%2Cb=6')
  })
  it('It sould support multi keys', function () {
    assert.deepStrictEqual(api._param({page: 1, size: 1}), 'page=1&size=1')
  })
})

describe('url combination', function () {
  it('It should support url combination', function () {
    assert.deepStrictEqual(api._join('foo', 'bar'), 'foo/bar')
  })
  ...
  it('It should support more than two parameters', function () {
    assert.deepStrictEqual(api._join('foo', 'bar', 'baz'), 'foo/bar/baz')
  })
})
